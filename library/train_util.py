...
def get_optimizer(args, trainable_params):
    ...
    if optimizer_type == "Lion".lower():
        ...
        optimizer = optimizer_class(trainable_params, lr=lr, **optimizer_kwargs)
    # ----------  AÑADE ESTE BLOQUE  ----------
    elif optimizer_type == "CAME".lower():               # ← string que llega del TOML
        try:
            import came_pytorch                          # pip install came-pytorch
        except ImportError:
            raise ImportError("Se pidió CAME pero came_pytorch no está instalado")
        print(f"use CAME optimiser | {optimizer_kwargs}")
        optimizer_class = came_pytorch.CAME            # ← nombre real de la clase
        optimizer = optimizer_class(trainable_params, lr=lr, **optimizer_kwargs)
    # ----------  FIN BLOQUE NUEVO  ----------
...
def get_scheduler_fix(args, optimizer: Optimizer, num_processes: int):
    ...
    if name == SchedulerType.COSINE_WITH_RESTARTS:
        # ----------  INICIO BLOQUE REX  ----------
        if getattr(args, "lr_scheduler_type", None) == "rex":
            # calculamos los pasos de ciclo y warmup por ciclo
            cycle_steps = num_training_steps // max(num_cycles, 1)
            warmup_steps_cycle = max(1, int(num_warmup_steps / max(num_cycles, 1)))
            lr_scheduler_kwargs.update(
                dict(
                    first_cycle_max_steps=cycle_steps,
                    warmup_steps=warmup_steps_cycle,
                    min_lr=1e-9,
                    gamma=0.9,
                    d=0.9,
                )
